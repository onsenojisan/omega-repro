...
These research areas are referenced as adjacent terminology, not as equivalents to Ω, explanations of Ω behavior, or replacements for domain-specific methods.
...
| Anomaly detection | Identifying observations that depart from expected patterns | Comparing concentration in event-adjacent or state-conditioned windows defined outside Ω | Ω can be reported alongside anomaly workflows, but it does not define anomalies by itself. |
...
| Volatility clustering | Persistence or grouping of high-variance periods | Measuring concentration structure under fixed windows and externally defined event or state labels | Ω may be reported alongside volatility patterns, but concentration is not the same as volatility. |
