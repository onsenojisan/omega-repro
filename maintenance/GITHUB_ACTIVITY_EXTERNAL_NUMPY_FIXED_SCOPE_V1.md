# GitHub Activity External NumPy Fixed Scope v1.0

Status: Internal / pre-data fixed scope  
Purpose: Fixed execution scope for second external GitHub activity Ω test  
...
This document fixes the execution scope for the second external-repository GitHub activity Ω structural concentration test.
...
numpy/numpy is a public, high-activity open-source repository with long-term commit history and sufficient expected daily activity for an external GitHub activity Ω test.

Role in the empirical sequence:

This is the second external-repository test after the fixed pandas-dev/pandas test produced a positive concentration result.
...
This test is not a correction or optimization of the pandas result.
...
The cutoff date matches the fixed cutoff used in the first internal GitHub activity pipeline test and the first external pandas test.
...
The second external test uses daily commit count only.
...
## 13. Relationship to previous GitHub activity tests
...
The first external GitHub activity test used:
...
This NumPy test is a second external-repository test under the same design family.

It is not a tuning response to the pandas result.

It must preserve the same fixed specification.
