"""Run the fixed external pandas GitHub activity Ω test.

This script follows maintenance/GITHUB_ACTIVITY_EXTERNAL_PANDAS_FIXED_SCOPE_V1.md.
It reads local git history from pandas-dev/pandas, prints the minimal result
block to stdout, and does not create result files.
"""
...
TARGET_REPOSITORY = "pandas-dev/pandas"
...
def run_git(args: list[str], cwd: Path) -> str:
    try:
        ...
        raise SystemExit("git executable not found; repository history is unavailable") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or exc.stdout or "").strip()
        raise SystemExit(f"git command failed in {cwd}: git {' '.join(args)}\n{detail}") from exc
...
def clone_pandas_repo(parent_dir: Path) -> Path:
    destination = parent_dir / "pandas"
    try:
        subprocess.run(
        ...
        raise SystemExit("git executable not found; cannot clone pandas-dev/pandas") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or exc.stdout or "").strip()
        raise SystemExit(f"failed to clone {TARGET_REPOSITORY} from {CLONE_URL}\n{detail}") from exc
    return destination


def require_pandas_git_history(repo_dir: Path) -> None:
    if not repo_dir.exists():
        raise SystemExit(f"provided repo directory does not exist: {repo_dir}")
    if not repo_dir.is_dir():
        raise SystemExit(f"provided repo path is not a directory: {repo_dir}")

    inside = run_git(["rev-parse", "--is-inside-work-tree"], repo_dir).strip()
    if inside != "true":
        raise SystemExit(f"not a git work tree: {repo_dir}")

    commit_count = run_git(["rev-list", "--count", "HEAD"], repo_dir).strip()
    if not commit_count.isdigit() or int(commit_count) == 0:
        raise SystemExit(f"no commits found in repository history: {repo_dir}")

    remote_url = run_git(["remote", "get-url", "origin"], repo_dir).strip()
    normalized = remote_url.lower().removesuffix(".git")
    if TARGET_REPOSITORY not in normalized:
        raise SystemExit(
            f"unexpected origin remote for fixed external test: {remote_url!r}; "
...
def load_commit_dates(repo_dir: Path) -> tuple[list[date], str]:
    author_output = run_git(["log", "--format=%aI"], repo_dir)
    ...
    committer_output = run_git(["log", "--format=%cI"], repo_dir)
...
def run_analysis(repo_dir: Path) -> None:
    require_pandas_git_history(repo_dir)
    commit_dates, timestamp_rule_used = load_commit_dates(repo_dir)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the fixed external pandas GitHub activity Ω test."
    )
    parser.add_argument(
        "--repo-dir",
        type=Path,
        help="Path to an existing local pandas-dev/pandas clone outside omega-repro.",
    )
...
def main() -> None:
    ...
    if args.repo_dir is not None:
        ...
        if is_inside(repo_dir, omega_repro_root()):
            raise SystemExit(
                "--repo-dir must point outside the omega-repro working tree to avoid "
                "leaving external repository data in tracked project space"
            )
        run_analysis(repo_dir)
        return

    with tempfile.TemporaryDirectory(prefix="pandas_github_activity_") as temp_root:
        repo_dir = clone_pandas_repo(Path(temp_root))
        run_analysis(repo_dir)
