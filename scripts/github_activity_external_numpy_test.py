"""Run the fixed external NumPy GitHub activity Ω test.

This script follows maintenance/GITHUB_ACTIVITY_EXTERNAL_NUMPY_FIXED_SCOPE_V1.md.
It reads local git history from numpy/numpy, prints the minimal result block
to stdout, and does not create result files.
"""
...
def clone_numpy_repo(parent_dir: Path) -> Path:
    destination = parent_dir / "numpy"
    try:
        subprocess.run(
        ...
        raise SystemExit("git executable not found; cannot clone numpy/numpy") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or exc.stdout or "").strip()
...
def require_numpy_git_history(repo_dir: Path) -> None:
...
def run_analysis(repo_dir: Path) -> None:
    require_numpy_git_history(repo_dir)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the fixed external NumPy GitHub activity Ω test."
    )
    parser.add_argument(
        "--repo-dir",
        type=Path,
        help="Path to an existing local numpy/numpy clone outside omega-repro.",
    )
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="numpy_github_activity_") as temp_root:
        repo_dir = clone_numpy_repo(Path(temp_root))
        run_analysis(repo_dir)
